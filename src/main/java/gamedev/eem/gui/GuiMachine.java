...
public class GuiMachine extends GuiContainer
{
    ...
    public GuiMachine(InventoryPlayer playerInv, IInventory machineInv)
    {
        super(new ContainerMachine(playerInv, machineInv));
        this.playerInventory = playerInv;
        this.tileMachine = machineInv;
    }
    ...
    protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY)
    {
        ...
        this.mc.getTextureManager().bindTexture(MACHINE_GUI_TEXTURES);
        int i = (this.width - this.xSize) / 2;
        int j = (this.height - this.ySize) / 2;
        this.drawTexturedModalRect(i, j, 0, 0, this.xSize, this.ySize);

        if (TileEntityMachine.isRunning())
        {
    ...
    private int getCookProgressScaled(int pixels)
    {
        ...
        int j = this.tileMachine.getField(3);
        return j != 0 && i != 0 ? i * pixels / j : 0;
    }

    private int getBurnLeftScaled(int pixels)
    {
        int i = this.tileMachine.getField(1);
        ...
        return this.tileMachine.getField(0) * pixels / i;
    }
}